...
import { Plus, MessageCircle, QrCode, Smartphone, Link2, CheckCircle2, XCircle, AlertTriangle, Loader2, Trash2, Key, Check, BadgeCheck, Phone, Wifi, Edit, X, Hash, Globe } from 'lucide-react';
...
export default function WhatsAppSettings({ setActiveTab }: WhatsAppSettingsProps) {
    ...
    const loadSettings = async () => {
      ...
      try {
        ...
        if (settings && settings.length > 0) {
          const accounts = settings.map((s: any) => {
          ...
          const activeIndex = settings.findIndex((s: any) => s.is_active || s.isActive);
  ...
  const handleAddAccount = async () => {
    ...
    try {
      ...
      const settings = await getWhatsAppSettings(user?.uid || '');
      setWhatsappAccounts(settings.map((s: any) => ({
        id: s.id,
      ...
      // Fetch account info to display and save
      try {
        const accountInfo = await fetchWhatsAppAccount(formattedInstanceId, newAccount.token);

        // Update the document we just created with the real info
        const settingsQ = query(collection(db, 'whatsapp_settings'),
          where('instance_id', '==', formattedInstanceId),
          where('token', '==', newAccount.token)
        );
        const settingsSnap = await getDocs(settingsQ);
        if (!settingsSnap.empty) {
          const docId = settingsSnap.docs[0].id;
          await updateDoc(doc(db, 'whatsapp_settings', docId), {
  ...
  // Fetch account info from the hook
  const handleFetchAccount = async (instanceId: string, token: string) => {
    ...
    try {
      ...
      // Persist the fetched info to Firestore if we have a document ID
      const activeAccount = whatsappAccounts[activeAccountIndex];
      if (activeAccount?.id && activeAccount.instanceId === instanceId) {
        updateDoc(doc(db, 'whatsapp_settings', account.id || activeAccount.id), {
          name: account.name || '',
          profile_picture: account.profile_picture || '',
          phone: account.phone || '',
          updated_at: new Date()
        }).catch(err => console.error('Error auto-syncing account info:', err));
      }
    } catch (error) {
          ...
          <div className="flex items-center justify-between relative z-10">
            ...
            <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-lg border border-gray-200 shadow-inner">
              <button
                ...
                className={`px-3 py-1.5 font-medium text-sm rounded-lg transition-colors ${activeSettingsTab === 'accounts'
              ...
              <button
                ...
                className={`px-3 py-1.5 font-medium text-sm rounded-lg transition-colors ${activeSettingsTab === 'templates'
                  ? 'bg-green-50 text-green-600 shadow-sm'
                  : 'text-gray-600 hover:bg-white'
                  }`}
              >
              ...
              <div className="bg-white rounded-lg p-5 border border-gray-200 hover:border-green-200 transition-all shadow-sm hover:shadow">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-4">
                    ...
                    {connectionStatus && (
                      <div className={`flex items-center gap-2 p-4 rounded-lg border ${connectionStatus === 'success'
