...
import { Building2, Phone, Box, FileText, TriangleAlert as AlertTriangle, DollarSign, CreditCard, ArrowUpLeft } from 'lucide-react';
...
const PaymentForm = forwardRef<FormRef, PaymentFormProps>(({ safes, companies, onClose }, ref) => {
    const { employee } = useAuth();
    const { currentRate } = useExchangeRate();
    const { createVoucher } = useVouchers({ type: 'payment' });
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-500">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h3 className="text-2xl font-black text-gray-900 dark:text-white">سند دفع</h3>
                    <p className="text-xs text-gray-500 mt-0.5">صرف مبالغ مالية لجهة محددة</p>
                </div>
            </div>
            ...
            <div className="space-y-8 pb-10">
                ...
                <div className="p-6 bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm space-y-4">
                    <div className="flex items-center justify-between">
                        <label className="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-1">
                            <Building2 className="w-3.5 h-3.5" />جهة الصرف (الحساب المستلم)
                        </label>
                        {formData.entityType && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-12 xl:col-span-5 space-y-6">
                        <div className="p-6 bg-rose-50/30 dark:bg-rose-900/10 rounded-3xl border border-rose-100 dark:border-rose-800/50 space-y-6 shadow-inner">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-1 block text-center">المبلغ المراد صرفه</label>
                            ...
                            {formData.currency === 'USD' && (
                                <div className="flex items-center justify-between p-3 rounded-xl bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100/50 dark:border-blue-800/50">
                                    <div className="flex flex-col">
                                        <span className="text-[9px] font-black text-blue-500 uppercase tracking-tighter">Exchange Rate</span>
                    ...
                    <div className="lg:col-span-12 xl:col-span-7 space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    className="w-full h-12 px-4 rounded-xl border-2 border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm font-bold focus:border-rose-500 transition-all outline-none shadow-sm"
                                    placeholder="964xxx"
                                    dir="ltr"
                                />
                            </div>
                            <div className="space-y-2">
                                <label className="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-1">
                                    <Box className="w-3.5 h-3.5" />يصرف من صندوق
                                </label>
                                <select
                                    ...
                                    className="w-full h-12 px-4 rounded-xl border-2 border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm font-bold focus:border-rose-500 transition-all outline-none shadow-sm cursor-pointer"
                                >
                        ...
                        <div className="space-y-2">
                            <label className="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-1">
                                <FileText className="w-3.5 h-3.5" />البيان (تفاصيل الصرف)
                            </label>
                            <textarea
                                value={formData.details}
                                onChange={e => setFormData(prev => ({ ...prev, details: e.target.value }))}
                                className="w-full h-32 px-4 py-3 rounded-2xl border-2 border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm font-bold focus:border-rose-500 transition-all resize-none outline-none shadow-sm"
                                placeholder="اكتب الغرض من صرف المبلغ هنا..."
                            />
                        </div>
                    </div>
                </div>
