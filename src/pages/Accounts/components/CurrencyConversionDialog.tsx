...
const CurrencyConversionDialog: React.FC<CurrencyConversionDialogProps> = ({
  ...
  const modalContent = (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[10000] animate-in fade-in duration-200" onClick={onClose}>
      <div
        className={`rounded-2xl shadow-2xl border p-6 max-w-md w-full mx-4 ${theme === 'dark'
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <div className={`p-3 rounded-xl shadow-lg ${theme === 'dark'
              ? 'bg-gradient-to-br from-blue-600 to-blue-700'
              : 'bg-gradient-to-br from-blue-100 to-blue-50'
              }`}>
              <DollarSign className={`w-6 h-6 ${theme === 'dark' ? 'text-white' : 'text-blue-600'
                }`} />
            </div>
            <div>
        ...
        <div className={`p-5 rounded-xl border mb-5 ${theme === 'dark'
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className={`p-4 rounded-lg border shadow-sm ${theme === 'dark'
              ...
              <div className={`text-sm mb-1 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-700'
                }`}>المبلغ بالدولار</div>
            ...
            <div className={`p-4 rounded-lg border shadow-sm ${theme === 'dark'
              ...
              <div className={`text-sm mb-1 ${theme === 'dark' ? 'text-green-400' : 'text-green-700'
                }`}>المبلغ بالدينار</div>
              <div className={`text-2xl font-bold ${theme === 'dark' ? 'text-green-300' : 'text-green-800'
                }`} dir="ltr">{convertedAmount.toLocaleString()} IQD</div>
            </div>
          </div>
          <div className="flex items-center justify-center mt-4 gap-2">
            <ArrowRight className={`w-5 h-5 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'
              }`} />
            <div className={`text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-blue-700'
              }`}>سعر الصرف:</div>
            <div className={`text-base font-bold ${theme === 'dark' ? 'text-blue-300' : 'text-blue-800'
              }`} dir="ltr">{rate.toLocaleString()} IQD</div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <label className="relative flex-1 cursor-pointer">
              ...
              />
              <div className={`flex items-center gap-3 p-4 border-2 rounded-xl transition-all ${!convertToIQD
                ? theme === 'dark'
                ...
                <div className={`p-2.5 rounded-lg ${!convertToIQD
                  ? 'bg-blue-600'
                  : theme === 'dark'
                    ? 'bg-gray-600'
                    : 'bg-blue-100'
                  }`}>
                  <DollarSign className={`w-5 h-5 ${!convertToIQD
                    ? 'text-white'
                    : theme === 'dark'
                      ? 'text-gray-300'
                      : 'text-blue-600'
                ...
                <div className="flex-1">
                  <div className={`font-bold ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'
                    }`}>إرسال بالدولار</div>
        ...
        <div className={`flex items-center justify-end gap-3 mt-6 pt-5 border-t ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'
          }`}>
          <button
            onClick={onClose}
            className={`px-5 py-2.5 rounded-lg font-medium transition-colors ${theme === 'dark'
