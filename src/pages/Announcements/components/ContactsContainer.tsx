...
import { Contact, Search, RefreshCw, CheckCircle2, ImageIcon, Trash2, Plus, Rocket, Loader2, UserX } from 'lucide-react';
...
const AnimatedContactCard = ({ contact, isSelected, onToggle }: any) => {
    return (
        <motion.div
        ...
        >
            ...
            <motion.div
                className={`absolute inset-0 z-0 origin-right ${isSelected ? 'bg-purple-500/20' : 'bg-purple-50/50 dark:bg-purple-500/10'}`}
                initial={{ scaleX: 0 }}
                whileHover={{ scaleX: 1 }}
            ...
            <button
                onClick={onToggle}
                className={`relative z-10 flex items-center gap-3 h-full px-4 text-right w-full transition-all ${isSelected ? 'ring-2 ring-purple-400/50' : ''
                    }`}
            >
                <div className="relative w-12 h-12 flex-shrink-0">
                    {contact.image ? (
                        <img src={contact.image} className="w-full h-full rounded-xl object-cover shadow-sm" alt="" />
                    ) : (
                        <div className={`w-full h-full rounded-xl flex items-center justify-center shadow-inner ${isSelected ? 'bg-purple-500 text-white' : 'bg-green-500/10 text-green-500 border border-green-500/20'
                            }`}>
                            <WhatsAppIcon className="w-6 h-6" />
                        </div>
                    )}
                    {isSelected && (
                        <div className="absolute -top-1 -right-1 w-5 h-5 bg-purple-500 text-white rounded-full flex items-center justify-center shadow-lg border-2 border-white dark:border-gray-900">
                            <CheckCircle2 className="w-3 h-3" />
                        </div>
                    )}
                </div>
                <div className="flex-1 min-w-0">
                    <p className={`text-sm font-black truncate transition-colors ${isSelected ? 'text-purple-600 dark:text-purple-400' : 'text-gray-700 dark:text-gray-200'}`}>
                        {contact.name || contact.phone}
                    </p>
                    <p className="text-[10px] text-gray-400 truncate mt-0.5 font-bold">{contact.id}</p>
                </div>
            </button>
        </motion.div>
    );
...
const ContactsContainer: React.FC<ContactsContainerProps> = ({
    ...
    const { whatsappContacts, isLoading, fetchContacts } = useWhatsAppContacts(false, selectedAccount);
    ...
    useEffect(() => {
        if (selectedAccount) fetchContacts();
    }, [selectedAccount, fetchContacts]);
    ...
    }, [whatsappContacts, searchTerm, exclusions]);

    const excludedCount = useMemo(() => {
        return whatsappContacts.filter(c => exclusions.has(c.id)).length;
    }, [whatsappContacts, exclusions]);
    ...
    const handleToggleAll = () => {
        ...
        if (allFilteredSelected) {
            ...
            onSelectAll(filteredContacts.map(c => ({ ...c, type: 'contact' })));
        }
    };
    ...
    const startBroadcast = async () => {
        ...
        try {
            ...
            const recipients = Array.from(selectedIds).map(id => {
                ...
                return { id: id, name: contact?.name || (contact as any)?.phone || 'Contact', phone: (contact as any)?.phone || id };
            });
        ...
        <div className="flex flex-col h-[780px] w-full overflow-hidden bg-white dark:bg-gray-950">
            ...
            <div className="px-6 py-2 flex flex-col md:flex-row items-center justify-between gap-6 bg-gradient-to-r from-[#2e1065] via-[#1e1b4b] to-[#0f172a] shadow-2xl sticky top-0 z-50 shrink-0">
                ...
                <div className="flex items-center gap-2 shrink-0 h-11 px-3 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-[inset_0_0_20px_rgba(255,255,255,0.02)] group/hud hover:border-white/20 transition-all duration-500">
                    ...
                    <div className="flex items-center gap-2 h-8 px-2.5 bg-purple-500/10 border border-purple-500/20 rounded-xl group-hover/hud:bg-purple-500/20 transition-all duration-500">
                        <Contact className="w-3.5 h-3.5 text-purple-400" />
                        <span className="text-[11px] font-black text-white/80 tabular-nums">{filteredContacts.length}</span>
                    </div>
                    {excludedCount > 0 && (
                ...
                <div className="lg:col-span-5 space-y-5 overflow-y-auto hide-scrollbar scroll-smooth">
                    ...
                    <div className="space-y-1">
                        <label className="text-[11px] font-black text-black dark:text-white uppercase tracking-widest flex items-center gap-2 px-2">
                            <ImageIcon className="w-4 h-4 text-indigo-500" /> مرفق الإعلان
                        </label>
                        <AnimatePresence mode="wait">
                            {imagePreview ? (
                                ...
                                <motion.label
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                                    className="flex flex-col items-center justify-center w-full h-[180px] bg-white dark:bg-gray-950 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-2xl cursor-pointer hover:bg-purple-50/30 dark:hover:bg-purple-500/5 transition-all group"
                                >
                                    <div className="p-3 bg-purple-50 dark:bg-purple-500/10 rounded-xl shadow-sm mb-2 group-hover:scale-110 transition-all">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 content-start pb-6">
                            {isLoading ? (
                                <div className="col-span-full py-20 flex flex-col items-center justify-center text-gray-400">
                                    <Loader2 className="w-12 h-12 animate-spin text-purple-500 mb-4 opacity-50" />
                                    <span className="text-sm font-black tracking-widest uppercase opacity-50">جاري تحميل جهات الاتصال...</span>
                                </div>
                                ...
                                filteredContacts.map(contact => (
                                    <AnimatedContactCard
                                        ...
                                        onToggle={() => onToggleTarget({ ...contact, type: 'contact' })}
                                    />
                                ))
                            ) : (
                                <div className="col-span-full py-20 text-center">
                                    <div className="w-20 h-20 bg-gray-50 dark:bg-gray-900 rounded-full flex items-center justify-center mx-auto mb-6">
                                        <Contact className="w-10 h-10 text-gray-200 dark:text-gray-800" />
                                    </div>
                                    <p className="text-sm font-black text-gray-300 dark:text-gray-700">لا توجد جهات اتصال تطابق البحث</p>
                                </div>
                            )}
                        </div>
                    </div>
