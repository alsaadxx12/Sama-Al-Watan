...
export default function CompanyCard({
  ...
  const getEntityBadge = () => {
    if (company.entityType === 'client') {
      return (
        <div className={`px-3 py-1 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-green-500/20 text-green-400' : 'bg-green-100 text-green-700'
          }`}>
          عميل
        </div>
      );
    ...
    if (company.entityType === 'expense') {
      return (
        <div className={`px-3 py-1 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-red-500/20 text-red-400' : 'bg-red-100 text-red-700'
          }`}>
          مصروف
        </div>
      );
    ...
    return (
      <div className={`px-3 py-1 rounded-full text-xs font-medium ${company.paymentType === 'cash'
          ? theme === 'dark'
  ...
  if (gridView === 'list') {
    return (
      <div className={`rounded-xl border overflow-hidden transition-all hover:shadow-lg ${theme === 'dark'
        ...
        <div className="p-5">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4 flex-1">
              <div className={`p-3 rounded-xl ${company.entityType === 'client'
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  {company.whatsAppGroupId && (
                    <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium ${theme === 'dark'
  ...
  return (
    <div className={`rounded-xl border overflow-hidden transition-all hover:shadow-lg ${gridView === 'compact' ? 'h-auto' : 'h-[240px]'
      } flex flex-col ${theme === 'dark'
        ? 'bg-gray-800/60 border-gray-700 hover:border-gray-600'
        : 'bg-white border-gray-200 hover:border-gray-300'
      }`}>
      ...
      <div className="p-5 flex-1 flex flex-col">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h3 className={`text-base font-bold leading-tight ${theme === 'dark' ? 'text-gray-100' : 'text-gray-800'
                }`}>{company.name}</h3>
        ...
        <div className="flex-1 space-y-2">
          {company.whatsAppGroupId && company.whatsAppGroupName && (
            <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium ${theme === 'dark'
                ? 'bg-green-900/30 text-green-400'
                : 'bg-green-50 text-green-700'
              }`}>
              <MessageCircle className="w-3.5 h-3.5" />
        ...
        <div className={`mt-auto pt-3 border-t flex items-center justify-between rounded-b-xl -mx-5 -mb-5 px-5 py-3 ${getFooterColor()}`}>
          <div className={`flex items-center gap-1.5 text-xs font-medium ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'
            }`}>
