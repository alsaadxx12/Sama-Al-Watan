...
const SafePhysicsCard: React.FC<SafePhysicsCardProps> = ({
    ...
    return (
        ...
        >
            <div
                className={`relative p-6 rounded-3xl border transition-all duration-300 group ${theme === 'dark'
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <AnimatePresence>
                        {isExpanded && safeUnconfirmed.length > 0 && (
                            ...
                            >
                                <div className={`mt-2 p-2 rounded-xl flex flex-col ${theme === 'dark' ? 'bg-gray-800/50' : 'bg-gray-50'
                                    }`}>
                                    <div className="max-h-[360px] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                                        {safeUnconfirmed.map(voucher => (
                                            <div key={voucher.id} className={`flex justify-between items-center p-2 rounded-lg text-xs ${theme === 'dark' ? 'bg-gray-700/50 hover:bg-gray-700' : 'bg-white border border-gray-100 hover:border-gray-200 shadow-sm'
                                                }`}>
                                                <span className={`font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                                                    {voucher.companyName || 'بدون اسم'}
                                                </span>
                                                <span className={`font-black ${voucher.currency === 'USD' ? 'text-emerald-500' : 'text-blue-500'
                                                    }`} dir="ltr">
                                    ...
                                    <div className="pt-2 flex justify-end">
                                        <button
                                            className="text-xs text-green-500 hover:text-green-600 font-black underline transition-colors"
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                onConfirmAll();
                                            }}
